...
public class SQLTemplate {
	...
	public static final String LIST_WORD_GAME1 = "select u.word_id, w.antonym, UNIX_TIMESTAMP(u.interactedTime) as time, u.totalNumberOfReview, u.totalNumberOfWrong from userword as u, word as w where u.word_id = w.id and u.user_id = %d";
	...
	public static final String UPDATE_TOTAL_TIME = "update user set user.total_time = user.total_time + %d where user.id = %d";
	public static final String LIST_LEARNED_WORD = "select userdeck.decks_id, UNIX_TIMESTAMP(userdeck.createdAt) as time from userdeck where userdeck.user_id = %d order by time";
